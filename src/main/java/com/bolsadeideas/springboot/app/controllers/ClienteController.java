...
public class ClienteController {
	...
	public String listar(@RequestParam(name = "page", defaultValue = "0") int page, Model model,
		...
		if(hasRole("ROLE_ADMIN")) {
			logger.info("Hola, ".concat(auth.getName()).concat(", ¡tenés acceso!"));
		} else {
			logger.info("Hola, ".concat(auth.getName()).concat(", ¡NO tenés acceso!"));
		}
		
		SecurityContextHolderAwareRequestWrapper securityContext = new SecurityContextHolderAwareRequestWrapper(request, "");
		
		if(securityContext.isUserInRole("ROLE_ADMIN")) {
			logger.info("Forma usando SecurityContextHolderAwareRequestWrapper: Hola, ".concat(auth.getName()).concat(", ¡tenés acceso!"));
		} else {
			logger.info("Forma usando SecurityContextHolderAwareRequestWrapper: Hola, ".concat(auth.getName()).concat(", ¡NO tenés acceso!"));
		}
		
		if(request.isUserInRole("ROLE_ADMIN")) {
			logger.info("Forma usando HttpServletRequest: Hola, ".concat(auth.getName()).concat(", ¡tenés acceso!"));
		} else {
			logger.info("Forma usando HttpServletRequest: Hola, ".concat(auth.getName()).concat(", ¡NO tenés acceso!"));
		}
	...
	public String crear(Map<String, Object> model, Locale locale) {
		...
		model.put("boton", messageSource.getMessage("text.cliente.form.boton.crear", null, locale));
		return "form";
	}
	...
	public String guardar(@Valid Cliente cliente, BindingResult result, Model model,
		...
		if (result.hasErrors()) {
			...
			model.addAttribute("boton", messageSource.getMessage("text.cliente.form.boton", null, locale));
			return "form";
		}
